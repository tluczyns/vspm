package tl.sound {
	...
	public class SoundGlobalVolumeRatio extends Object {
		...
		function SoundGlobalVolumeRatio(soName: String, stepChangeVolumeRatio: Number = 0.05): void {
			...
			ModelSoundEngine.volumeRatio = this.so ? this.so.getPropValue("volumeRatio", 1) : 1;
		...
		private function onVolumeRatioChanged(e: EventSoundEngine): void {
			...
			if (this.so) this.so.setPropValue("volumeRatio", ModelSoundEngine.volumeRatio);
		}
		...
		private function onTweenVolumeRatioChanged(e: EventSoundEngine): void {
			...
			Tweener.addTween(ModelSoundEngine, {volumeRatio: ModelSoundEngine.tweenVolumeRatio, time: numFramesChangeVolumeRatio, useFrames: true, transition: "linear"});
			if (this.so) this.so.setPropValue("volumeRatio", String(ModelSoundEngine.tweenVolumeRatio));
		}
